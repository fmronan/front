module.exports = function(grunt) {
    ...
    var config = {
        ...
        shell: {
            ...
            removeLogs: {
                ...
                command: 'rm -Rf var/logs/<%= cfg.environment %>.log'
            },
            rightsLogs: {
                options: {
                    stdout: true
                },
                command: 'chown -Rf <%= cfg["deploy-user"] %>:<%= cfg["deploy-group"] %> var/logs/ && chmod 775 var/logs/ && find var/logs/ -type f -exec chmod 664 {} \\;'
            },
            rightsSession: {
                options: {
                    stdout: true
                },
                command: 'chown -Rf <%= cfg["deploy-user"] %>:<%= cfg["deploy-group"] %> var/session/dev/ && chmod 775 var/session/dev/ && find var/session/dev/ -type f -exec chmod 664 {} \\;'
            },
            rightsSitemap: {
                options: {
                    stdout: true
            ...
            removeCache: {
                ...
                command: 'rm -Rf var/cache/<%= cfg.environment %>_old && rm -Rf var/cache/<%= cfg.environment %>/*'
            },
            clearCache: {
                options: {
                    stdout: true
                },
                command: 'php bin/console cache:clear --env=<%= cfg.environment %> <%= cfg["symfony-command-suffix"] %>'
            },
            rightsCache: {
                options: {
                    stdout: true
                },
                command: 'chown -Rf <%= cfg["deploy-user"] %>:<%= cfg["deploy-group"] %> var/cache/<%= cfg.environment %>/ && find var/cache/<%= cfg.environment %>/ -type d -exec chmod 775 {} \\; && find app/cache/<%= cfg.environment %>/ -type f -exec chmod 664 {} \\;'
            },
            assetsInstall: {
                options: {
                    stdout: true
                },
                command: 'php bin/console assets:install --env=<%= cfg.environment %> <%= cfg["symfony-command-suffix"] %>'
            },
            composerUpdate: {
                options: {
                    stdout: true
            ...
            jsRoutingDump: {
                ...
                command: 'php bin/console fos:js-routing:dump --env=<%= cfg.environment %> --locale=fr__RG__'
            },
            sitemapDump: {
                options: {
                    stdout: true
    ...
    grunt.registerTask('rightsSession', 'Command line to set rights on spool directory', function() {
        if (process.platform != 'win32') {
            grunt.task.run('shell:rightsSession');
        } else {
    ...
    grunt.registerTask('rightsApplication', 'Command line to set rights on application', function() {
        grunt.task.run(['applicationOwner', 'rightsCache', 'rightsLogs', 'rightsSession', 'rightsSitemap']);
    });
